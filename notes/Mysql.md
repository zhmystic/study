...
举例子：假如我现在通过name字段查询，name字段本身是没有索引的，我查询name=1的数据，这时候另一个session去修改name=2的数据，虽然说InnoDB默认是行级锁，不应该影响这种不是同一行的数据，但是因为name字段没有索引，所以还是会被锁住。假如name有索引的话，就没关系，是行级锁。**
...
真正达到防止幻读目的的，是因为事务对数据加了next-key锁。
next-key锁分为两部分：

* 行锁（Record Lock）
* Gap lOCK（间隙锁）
    * 防止同一事务的两次当前读出现幻读的情况。在RC及以下级别是没有的，所以无法避免幻读。在RR和SER以上都有。
...
GAP间隙锁会用在非唯一索引或者不走索引的当前读以及仅命中检索条件的部分结果集。
